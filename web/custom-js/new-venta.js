...
function mitad_pago() {
  ...
  if ($("#chkPagado").prop("checked") == true) {
    if (
      parseFloat(sumatoria) > parseFloat(deuda) ||
      parseFloat(sumatoria) < parseFloat(deuda)
    ) {
      $("#btnRegistrar").prop("disabled", true);
      $("#txtCambio").val("0.00");
...
function Venta_Credito() {
  ...
  if (
    parseFloat(monto_deudor) > parseFloat(limite_credito) ||
    parseFloat(limite_credito) == 0.0 ||
    limite_credito == ""
  ) {
    $("#btnRegistrar").prop("disabled", true);
  } else {
...
function agregar_detalle(
  ...
  if (inventariable == 0) {
    ...
    tr_add +=
      ...
      "</span></td>";
    tr_add +=
      '<td width="5%"><input type="text" id="tblcant" name="tblcant" value="1.00" class="touchspin" style="width:70px;"></td>';
    tr_add += '<td align="center">' + precio_venta + "</td>";
    tr_add += '<td align="center">' + exento + "</td>";
    tr_add +=
      '<td width="5%"><input type="text" id="tbldesc" name="tbldesc"  value="0.00" class="touchspin" style="width:70px;"></td>';
    tr_add += '<td align="center">' + precio_venta + "</td>";
    tr_add += '<td align="center">/</td>';
    tr_add +=
      '<td align="center" class="Delete"><button type="button"class="btn btn-link btn-xs"><i class="icon-trash-alt"></i></button></td>';
    tr_add += "</tr>";
    ...
    if (perecedero == 0) {
      ...
      select_fechas =
        "<select id='cbFecha' name='cbFecha' class='form-control'>";
      //  $('.select-size-xs').select2();
      $.getJSON(
        "web/ajax/ajxventa.php?idproducto=" + idproducto,
        function (datos) {
          if (datos.length > 0) {
...
function setRowCant(rowId) {
  var cantidad_anterior = $("#tbldetalle tr:nth-child(" + rowId + ")")
    .find("td:eq(2)")
    .find("#tblcant")
    .val();
  var cantidad_nueva = parseFloat(cantidad_anterior) + 1;
  $("#tbldetalle tr:nth-child(" + rowId + ")")
    .find("td:eq(2)")
    .find("#tblcant")
    .val(cantidad_nueva.toFixed(2));
  totales();
}
        ...
        $.each(data, function (key, val) {
          ...
          $("#tbldetalle tbody tr").each(function (index) {
            ...
            if (total_sumas >= monto_retencion) {
              ...
              total =
                subtotal + total_exentos - iva_retenido - total_descuentos;
            } else {
              total = subtotal + total_exentos - total_descuentos;
            }
          });

          if (isNaN(total_sumas)) {
            total_sumas = 0.0;
          }
          if (isNaN(total_iva)) {
            total_iva = 0.0;
...
function enviar_data() {
    ...
    $(this)
      ...
      .each(function (index2) {
        switch (index2) {
          ...
          case 7:
            campo7 = $(this).find("#cbFecha option:selected").text();
            fecha_vence = campo7;
            break;
        }
        //  $(this).css("background-color", "#ECF8E0");
  ...
  if (total > 0.0) {
    $.ajax({
      ...
      success: function (data) {
        if (data == "Validado") {
          ...
          swal(
            "Lo sentimos, no pudimos registrar tu informacion!",
    ...
    swal(
      "Imposible",
      "No se puede registrar una compra con valor 0.00",
      "warning"
    );
  }
}
